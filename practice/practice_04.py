...
import numpy as np
from load_util import load_one_stock,load_dates,load_matrix
...
# print(max_price, max_date, min_price, min_date)
...
"""
# matrix = load_matrix()
# stock_means = matrix.mean(axis=1, keepdims=True)
# date_means = matrix.mean(axis=0)
# print(f"종목별 평균가 : {stock_means.shape}")
# print(f"날짜별 평균가 : {date_means.shape}")
# print(f"각 종목에서 자기 평균을 뺀 배열 : {(matrix - stock_means).shape}")
